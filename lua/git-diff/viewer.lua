...
local ns = vim.api.nvim_create_namespace("git_diff")
...
local function build_file_tree(files)
	...
	for _, f in ipairs(files or {}) do
		...
		for i, part in ipairs(vim.split(f.path or "", "/", { plain = true })) do
			...
			if i == #vim.split(f.path or "", "/", { plain = true }) then
				existing.file = f
			end
			node = existing
		end
...
local function open_diff_window(title, body, opts)
	...
	local function render_left(content_w)
		...
		for _, row in ipairs(rendered_rows) do
			...
			if row.kind == "dir" then
				...
				table.insert(
					extmarks,
					{ line = lidx, col_start = add_start, col_end = add_start + #add_str, hl = "LocalDiffOk" }
				)
				table.insert(extmarks, {
					line = lidx,
					col_start = add_start + #add_str + 1,
					col_end = add_start + #add_str + 1 + #del_str,
					hl = "LocalDiffError",
				})
			end
		end
		...
		for _, e in ipairs(extmarks) do
			pcall(
				...
				e.line,
				e.col_start,
				{ end_col = e.col_end, hl_group = e.hl }
			)
		end
	...
	local close = function()
		...
		if state.left_win == left_win then
			state.left_win = nil
		end
		if state.right_win == right_win then
			state.right_win = nil
		end
	end
	...
	local function jump_right_to_file(f, focus)
		...
		for j = f._raw_start + 1, #body_lines do
			if body_lines[j]:match("^diff %-%-git ") then
				break
			end
			if body_lines[j]:match("^@@") then
				target = j
				break
			end
		end
	...
	update_diff_winbar = function()
		...
		local topline = vim.api.nvim_win_call(right_win, function()
	...
	local function open_at_cursor()
		if not opts.cwd or not (right_win and vim.api.nvim_win_is_valid(right_win)) then
			return
		end
		local body_row = vim.api.nvim_win_get_cursor(right_win)[1]
		if body_row < 1 or body_row > #body_lines then
	...
	if opts.refresh_fn then
		vim.keymap.set("n", "r", refresh, ro)
	end
	if opts.cwd then
		vim.keymap.set("n", "<CR>", open_at_cursor, ro)
	end
	vim.keymap.set("n", "<Tab>", function()
		if not left_win or not vim.api.nvim_win_is_valid(left_win) then
			toggle_files()
