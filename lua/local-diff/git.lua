...
local function fetch_diff(callback, target_override)
  ...
  vim.system({ 'git', '-C', cwd, 'rev-parse', '--is-inside-work-tree' }, { text = true }, function(check)
    vim.schedule(function()
      if check.code ~= 0 then callback(nil, 'Not inside a git repository', nil); return end
      ...
      local function with_target(target, label)
        ...
        vim.system({ 'git', '-C', cwd, 'merge-base', target, 'HEAD' }, { text = true }, function(mb)
          vim.schedule(function()
            local diff_target = (mb.code == 0 and mb.stdout and mb.stdout ~= '') and vim.trim(mb.stdout) or target

            vim.system({ 'git', '-C', cwd, 'log', '-1', '--format=%h%x09%s', diff_target }, { text = true }, function(lg)
              vim.schedule(function()
                if lg.code == 0 and lg.stdout and lg.stdout ~= '' then
                  local sha, subject = vim.trim(lg.stdout):match '^(%S+)\t(.*)$'
                  if sha then
                    if subject and #subject > 60 then subject = subject:sub(1, 57) .. '…' end
                    if label == sha or label:sub(1, #sha) == sha then
                      resolved_label = string.format('%s "%s"', sha, subject or '')
                    else
                      resolved_label = string.format('%s @ %s "%s"', label, sha, subject or '')
                    end
                  end
                end
                done_inner()
        ...
        vim.system({ 'git', '-C', cwd, 'ls-files', '--others', '--exclude-standard' }, { text = true }, function(ls)
          vim.schedule(function()
            if ls.code ~= 0 then results.untracked = ''; done_inner(); return end
            local untracked = {}
            for f in (ls.stdout or ''):gmatch '[^\n]+' do table.insert(untracked, f) end
            if #untracked == 0 then results.untracked = ''; done_inner(); return end
            local remaining = #untracked
            local chunks = {}
            for i, file in ipairs(untracked) do
              vim.system({ 'git', '-C', cwd, 'diff', '--no-index', '--', '/dev/null', file }, { text = true }, function(d)
                vim.schedule(function()
                  chunks[i] = ((d.code == 0 or d.code == 1) and d.stdout and d.stdout ~= '') and d.stdout or ''
                  remaining = remaining - 1
                  if remaining == 0 then
                    local parts = {}
                    for j = 1, #untracked do
                      if chunks[j] and chunks[j] ~= '' then table.insert(parts, chunks[j]) end
                    end
...
local function list_branches(callback)
  ...
  vim.system(
    ...
    function(obj)
      vim.schedule(function()
        if obj.code ~= 0 then callback({}); return end
        local branches, seen = {}, {}
...
local function pick_branch(callback)
  list_branches(function(branches)
    if #branches == 0 then vim.notify('No branches found', vim.log.levels.WARN); return end
      ...
      pickers.new({}, {
        ...
        attach_mappings = function(prompt_bufnr)
          actions.select_default:replace(function()
            ...
            if val and val ~= '' then callback(val) end
          end)
          return true
        end,
      }):find()
...
local function list_commits(callback)
  local cwd = vim.fn.getcwd()
  vim.system(
    { 'git', '-C', cwd, 'log', '-n', '50', '--date=format:%Y-%m-%d %H:%M', '--pretty=format:%h\31%cd\31%cr\31%s\31%an' },
    { text = true },
    function(obj)
      vim.schedule(function()
        if obj.code ~= 0 then callback({}); return end
        ...
        for line in (obj.stdout or ''):gmatch '[^\n]+' do
          ...
          if sha then table.insert(commits, { sha = sha, date = date, age = age, subject = subject, author = author }) end
        end
        callback(commits)
      end)
    end
...
local function pick_commit(callback)
  list_commits(function(commits)
    if #commits == 0 then vim.notify('No commits found', vim.log.levels.WARN); return end
    local function fmt(c) return string.format('%s  %s  %s  · %s', c.sha, c.date, c.subject, c.age) end
    local ok, pickers = pcall(require, 'telescope.pickers')
    if ok then
      local finders = require 'telescope.finders'
      local conf = require('telescope.config').values
      ...
      pickers.new({}, {
        ...
        finder = finders.new_table {
          ...
          entry_maker = function(c)
            return { value = c.sha, display = fmt(c), ordinal = c.sha .. ' ' .. c.subject .. ' ' .. c.author }
          end,
        },
        sorter = conf.generic_sorter {},
        attach_mappings = function(prompt_bufnr)
          actions.select_default:replace(function()
            local entry = action_state.get_selected_entry()
            actions.close(prompt_bufnr)
            if entry and entry.value then callback(entry.value) end
          end)
          return true
        end,
      }):find()
      ...
      vim.ui.select(items, { prompt = 'Diff vs commit' }, function(_, idx)
        if idx and commits[idx] then callback(commits[idx].sha) end
      end)
    end
  end)
end
...
local function open(target_override)
  local viewer = require 'local-diff.viewer'
  if viewer.is_open() then viewer.focus(); return end
  ...
  fetch_diff(function(diff, err, label)
    ...
    local cwd = vim.fn.getcwd()
    viewer.open(
      string.format('%s · local vs %s', vim.fn.fnamemodify(cwd, ':t'), label),
      diff,
      {
        cwd = cwd,
        refresh_fn = function(cb)
          fetch_diff(function(new_diff, new_err) cb(new_diff, new_err) end, target_override)
        end,
        initial_cursor = cursor_state,
        on_close = function(cur) cursor_state = cur end,
      }
    )
  end, target_override)
end
...
function M.show_with_prompt()
  vim.ui.select({ 'develop', 'main', 'Branch…', 'Commit…' }, { prompt = 'Diff vs:' }, function(choice)
    ...
    if choice == 'develop' or choice == 'main' then
      resolve_branch_ref(vim.fn.getcwd(), choice, function(target) open(target) end)
    elseif choice == 'Branch…' then
      pick_branch(function(target) open(target) end)
    elseif choice == 'Commit…' then
      pick_commit(function(target) open(target) end)
    end
  end)
end
